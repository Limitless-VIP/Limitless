...
*Setup instructions for a gitian build of TittieCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TittieCoin
...
- [Building TittieCoin](#building-tittiecoin)
...
Clone the git repositories for tittiecoin and gitian and then checkout the tittiecoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tittiecoin-crypto/tittiecoin.git
cd tittiecoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tittiecoin repository
...
To build TittieCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the tittiecoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tittiecoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tittiecoin-crypto/tittiecoin
...
For example:
```bash
URL=https://github.com/crowning-/tittiecoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit tittiecoin=${COMMIT} --url tittiecoin=${URL} ../tittiecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tittiecoin=${COMMIT} --url tittiecoin=${URL} ../tittiecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tittiecoin=${COMMIT} --url tittiecoin=${URL} ../tittiecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tittiecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tittiecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tittiecoin-build.assert
```
...
[tittiecoin/gitian.sigs](https://github.com/tittiecoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
