...
CzcTracker::CzcTracker(std::string strWalletFile)
{
...
CzcTracker::~CzcTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzcTracker::Init()
{
...
bool CzcTracker::Archive(CMintMeta& meta)
{
...
bool CzcTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzcTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzcTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzcTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzcTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzcTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzcTracker::GetSerialHashes()
{
...
CAmount CzcTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzcTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzcTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzcTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzcTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzcTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzcTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzcTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzcTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzcTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzcTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzcWallet* zVIPWallet)
{
    bool iszVIPWalletInitialized = (NULL != zVIPWallet);
    CMintMeta meta;
    ...
    if (! iszVIPWalletInitialized)
        zVIPWallet = new CzcWallet(strWalletFile);
    meta.isSeedCorrect = zVIPWallet->CheckSeed(dMint);
    if (! iszVIPWalletInitialized)
        delete zVIPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzcTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzcTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzcTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzcTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzcTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzcTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzcWallet* zVIPWallet = new CzcWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zVIPWallet);
        }
        delete zVIPWallet;
