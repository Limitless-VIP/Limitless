...
CzcWallet::CzcWallet(std::string strWalletFile)
{
...
bool CzcWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCCount(nCountLastUsed);
    else if (!walletdb.ReadZCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzcWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzcWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzcWallet::LoadMintPoolFromDB()
{
...
void CzcWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzcWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzcWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzcWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzcWallet::SeedToZC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzcWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzcWallet::GenerateDeterministicZC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzcWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzcWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzcWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
