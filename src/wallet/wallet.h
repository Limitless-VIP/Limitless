...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT15000IFMN = 3,
    ONLY_NONDENOMINATED_NOT15000IFMN = 4, // ONLY_NONDENOMINATED and not 15000 VIP at the same time
...
enum ZerocoinSpendStatus {
    ZC_SPEND_OKAY = 0,                            // No error
    ZC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZC_SPENT_USED_ZC = 14,                     // Coin has already been spend
    ZC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzvipAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzcTracker> zvipTracker;
    ...
    void setZWallet(CzcWallet* zwallet)
    {
        zwalletMain = zwallet;
        zvipTracker = std::unique_ptr<CzcTracker>(new CzcTracker(strWalletFile));
    }

    CzcWallet* getZWallet() { return zwalletMain; }
