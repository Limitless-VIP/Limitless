...
public:
    ...
    bool WriteZCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZCSeed_deprecated(uint256& seed);
    bool EraseZCSeed();
    bool EraseZCSeed_deprecated();

    bool WriteZCCount(const uint32_t& nCount);
    bool ReadZCCount(uint32_t& nCount);
