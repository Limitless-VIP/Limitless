...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Limitless address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Limitless: URI
...
{
    // Convert limitless:// to limitless:
    //
    //    Cannot handle this later, because limitless:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open limitless.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Limitless.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Limitless.lnk
...
{
    return GetAutostartDir() / "limitless.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a limitless.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Limitless\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the limitless app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add limitless app to startup item list
