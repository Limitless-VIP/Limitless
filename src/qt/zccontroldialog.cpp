...
#include "zccontroldialog.h"
#include "ui_zccontroldialog.h"
...
bool CZVipControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZVipControlDialog::COLUMN_DENOMINATION || column == ZVipControlDialog::COLUMN_VERSION || column == ZVipControlDialog::COLUMN_CONFIRMATIONS)
...
void ZVipControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZVipControlWidgetItem* itemDenom(new CZVipControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZVipControlWidgetItem *itemMint = new CZVipControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zvipTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zvipTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zVIP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zVIP seed used to mint this zVIP is not the same as currently hold in the wallet";
            else
