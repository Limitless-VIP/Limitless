...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LimitlessMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("LimitlessMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("LimitlessMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("limitless-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LimitlessMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
