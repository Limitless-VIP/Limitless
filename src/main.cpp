...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zVIP without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zVIP v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZCModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZCModule::validateInput(txin, prevOut, tx, ret)){
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state)
{
    ...
    // Size limits
    unsigned int nMaxSize = fZerocoinActive ? MAX_ZEROCOIN_TX_SIZE : MAX_BLOCK_SIZE_LEGACY;
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZCModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zVIP mints (if included) are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCStake)
{
    int64_t ret = 0;
...
void RecalculateZCMinted()
{
    ...
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted zVIP..."), percent);
        }
...
void RecalculateZCSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_Block_V2_Start()];
    uiInterface.ShowProgress(_("Recalculating spent ZC..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            ...
            uiInterface.ShowProgress(_("Recalculating spent zVIP..."), percent);
        }
...
bool RecalculateVIPSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating VIP supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating VIP supply..."), percent);
        }
...
bool UpdateZCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZCSupply(block, pindex, fJustCheck))
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> vipInputs;
        std::vector<CTxIn> zVIPInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zVIPInputs.push_back(stakeIn);
            }else{
                vipInputs.push_back(stakeIn);
            }
        }
        const bool hasVIPInputs = !vipInputs.empty();
        const bool hasZCInputs = !zVIPInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                if(nHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZCModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if(hasVIPInputs)
                    ...
                    for (const CTxIn& vipIn : vipInputs){
                        if(vipIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : vipInputs) {
            ...
            // Now that this loop if completed. Check if we have zVIP inputs.
            if(hasZCInputs){

                for (const CTxIn& zVipInput : zVIPInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zVipInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvip");
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zvip", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zvip", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
...
int ActiveProtocol()
{
    // SPORK_14 was used for 70712 (v2.x.x+).
    //if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
    //        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 is used for 70713 (v3.0.0+).
